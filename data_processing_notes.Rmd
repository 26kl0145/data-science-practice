...
* Be sure you spell the column name correctly (and the name if it's a categorical variable). Remember R is case-sensitive
* Be sure to use `==` when comparing observations. (Remember, `=` is an assignment operator)
* You can use >, <, >=, <=, to ocmpare numeric or categorical variables (nominal variables are ranked alphabetically, while ordinal variables have a built-in rank)
...
# %in% just means "is in". Help save time when using a bunch of or's for the same variable
importantPlanets <- c("Tatooine", "Naboo", "Hoth")
filter(starwars, homeworld %in% importantPlanets)
filter(starwars, homeworld %in% "Tatooine")
...
factor(starwars$eye_color,
       c("red", "orange", "gold", "yellow", "green yellow", "blue", "black"), ordered = T)

## Find all characters that are shorter than 100 cm
filter(starwars, height < 100, species != "Droid")

## Which characters were born between the years 100 and 200
...
`select` allows us to retain only certain variables (columns). It doesn't change the order
...
head(starwars)
select(starwars, hair_color:eye_color) # returns every column between first:last (including)
...
select(starwars, starts_with("s"), ends_with("color")) # for select, a comma means "or"
...
## Brainstorm as many ways a possible to select hair_color, eye_color, skin_color, and birth_year
select(starwars, ends_with("color"), ends_with("year"))
select(starwars, ends_with("r"), -gender)
select(starwars, contains("_"))
...
mutate(starwars, mean_mass = mean(mass, na.rm = T)) # This makes every value in mean_mass the same

summarise(starwars, mean_mass = mean(mass, na.rm = T)) # This condenses every value in mean_mass together   # na.rm means "remove na values", which we want to be true
...
starwars %>%
  group_by(gender) %>%
  sample_n(4) # This samples 4 people from each gender
